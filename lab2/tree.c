...
void print_tree(struct node* mnode, int depth)  // 打印AST，depth表示深度
{
    ...
    printf("\n");
    if(mnode->tokenFlag==0){
        for(int i=0;i<mnode->childnum;i++){
            print_tree(mnode->childs[i],depth+1);
        }
    }
...
//    mnode->child = NULL;
    ...
    for(int i=0;i<node_num;i++){
        mnode->childs[i]=va_arg(valist,struct node*);
    }
//    set_parent_brother(mnode, node_num, valist);
    va_end(valist);//结束对可变参数的处理。
    return mnode;
}
Node getchild(Node mnode,int child_idx){
    return mnode->childs[child_idx];
}
